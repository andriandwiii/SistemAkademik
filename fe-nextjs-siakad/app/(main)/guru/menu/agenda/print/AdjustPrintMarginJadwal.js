...
export default function AdjustPrintMarginJadwal({
  ...
  const generatePDF = () => {
    ...
    const tableRows = jadwalToPrint.map((jadwal) => {
      ...
      // Debug row pertama
      if (jadwalToPrint.indexOf(jadwal) === 0) {
        console.log('=== ROW PERTAMA ===');
        console.log('Raw jadwal:', jadwal);
        console.log('Processed row:', row);
        console.log('Hari value:', row['hari.HARI']);
      }
  ...
  const handleGenerate = () => {
    ...
    console.log('=== DEBUG PRINT JADWAL ===');
    console.log('Total data:', jadwalToPrint.length);
    console.log('Sample data pertama:', jadwalToPrint[0]);
    console.log('Struktur hari:', jadwalToPrint[0]?.hari);
    console.log('Nilai HARI:', jadwalToPrint[0]?.hari?.HARI);
