...
export default function AdjustPrintMarginLaporanMapelKelas({
  adjustDialog,
  setAdjustDialog,
  dataMapelKelas = [],
  setPdfUrl,
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN MAPEL-KELAS', marginLeft, marginTop, marginRight)

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Kelas', 'Mapel', 'Guru Pengampu', 'Kode/Ket']],
      body: dataMapelKelas.map((item) => [
        item.MAPEL_KELAS_ID,
        item.kelas
          ? `${item.kelas.TINGKATAN} ${item.kelas.NAMA_JURUSAN} ${item.kelas.NAMA_RUANG || '-'}`
          : '-',
        item.mapel
          ? `${item.mapel.KODE_MAPEL} - ${item.mapel.NAMA_MAPEL}`
          : '-',
        item.guru
          ? `${item.guru.GELAR_DEPAN ? item.guru.GELAR_DEPAN + ' ' : ''}${item.guru.NAMA_GURU}${item.guru.GELAR_BELAKANG ? ', ' + item.guru.GELAR_BELAKANG : ''}`
          : '-',
        item.KODE_MAPEL || '-',
      ]),
  ...
  // 🔸 Export Excel
  const exportExcel = () => {
    const dataForExcel = dataMapelKelas.map((item) => ({
      ID: item.MAPEL_KELAS_ID,
      'Kelas': item.kelas
        ? `${item.kelas.TINGKATAN} ${item.kelas.NAMA_JURUSAN} ${item.kelas.NAMA_RUANG || '-'}`
        : '-',
      ...
      'Guru Pengampu': item.guru
        ? `${item.guru.GELAR_DEPAN ? item.guru.GELAR_DEPAN + ' ' : ''}${item.guru.NAMA_GURU}${item.guru.GELAR_BELAKANG ? ', ' + item.guru.GELAR_BELAKANG : ''}`
        : '-',
      'Kode/Keterangan': item.KODE_MAPEL || '-',
    }))

    const ws = XLSX.utils.json_to_sheet(dataForExcel)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Data Mapel-Kelas')
    XLSX.writeFile(wb, 'Laporan_Mapel_Kelas.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Mapel_Kelas')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Mapel-Kelas"
      style={{ width: '50vw' }}
      modal
    >
