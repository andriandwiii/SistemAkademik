...
export default function AdjustPrintMarginJamPelajaran({
  adjustDialog,
  setAdjustDialog,
  dataJadwal = [],
  setPdfUrl,
  ...
  const exportPDF = async (adjustConfig) => {
    ...
    autoTable(doc, {
      startY,
      head: [['ID', 'Kode JP', 'JP Ke', 'Jam Mulai', 'Jam Selesai', 'Durasi', 'Deskripsi']],
  ...
  const exportExcel = () => {
    const dataForExcel = dataJadwal.map((j) => ({
      ...
      'Kode JP': j.KODE_JP,
      'JP Ke': j.JP_KE,
      'Jam Mulai': j.WAKTU_MULAI,
      'Jam Selesai': j.WAKTU_SELESAI,
      Durasi: j.DURASI,
    ...
    XLSX.writeFile(wb, 'Laporan_Jam_Pelajaran.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Jam_Pelajaran')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Jam Pelajaran"
      style={{ width: '50vw' }}
      modal
      blockScroll
    >
