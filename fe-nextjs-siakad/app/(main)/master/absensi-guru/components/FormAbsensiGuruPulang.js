...
const FormAbsensiGuruPulang = ({
  ...
  const sigCanvas = useRef(null); // Ref untuk Tanda Tangan
  ...
  useEffect(() => {
    if (visible) {
      setSelectedNip(defaultNip || null);
      setLatitude(null);
      setLongitude(null);
  ...
  const handleAmbilLokasi = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setLoadingLokasi(false);
        toast.current?.show({ severity: 'success', summary: 'Lokasi Terkunci', detail: 'Koordinat berhasil diambil.' });
      },
      (error) => {
        console.error(error);
        toast.current?.show({ severity: 'warn', summary: 'Gagal', detail: 'Pastikan GPS aktif & izinkan akses lokasi.' });
        setLoadingLokasi(false);
      },
  ...
  const handleSubmit = () => {
    // Validasi NIP
    if (!selectedNip) {
      toast.current?.show({ severity: 'warn', summary: 'Peringatan', detail: 'Pilih Guru terlebih dahulu.' });
      return;
    }
    
    // Validasi Lokasi (Wajib ada untuk LOKASI_KELUAR)
    if (!latitude || !longitude) {
       toast.current?.show({ severity: 'warn', summary: 'Lokasi Kosong', detail: 'Silakan klik tombol refresh lokasi.' });
       return;
    }

    // Validasi Tanda Tangan (Wajib ada untuk TANDA_TANGAN_KELUAR)
    ...
    const payload = {
        ...
        LATITUDE: latitude,
        LONGITUDE: longitude,
        TANDA_TANGAN_KELUAR: ttdBase64, // Kirim TTD Base64
    };
    ...
    >
      ...
      <div className="flex flex-column gap-3">
        ...
        <div className="text-center mb-1">
            ...
            <p className="text-gray-600 m-0 text-sm">Pastikan data lokasi dan tanda tangan sudah benar.</p>
        </div>
        ...
        <div className="field">
          ...
          <Dropdown
            ...
            filter
            className="w-full"
            disabled={!!defaultNip} // Disable jika NIP sudah terpilih otomatis
            appendTo="self"
          />
        </div>
        ...
        <div className="field">
            ...
            {latitude && <small className="text-green-600"><i className="pi pi-check-circle"></i> Lokasi siap disimpan.</small>}
        </div>

        {/* 3. TANDA TANGAN DIGITAL (CANVAS) */}
        <div className="field">
