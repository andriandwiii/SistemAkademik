...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(
      doc,
      'LAPORAN MASTER TINGKATAN SEKOLAH',
      marginLeft,
      marginTop,
      marginRight
    )
    ...
    autoTable(doc, {
      startY,
      head: [['ID', 'Kode Tingkatan', 'Tingkatan', 'Status']],
      body: dataTingkatan.map((t) => [
        t.id,
        t.TINGKATAN_ID || '-',
        t.TINGKATAN || '-',
        t.STATUS || '-',
  ...
  // 🔸 Export Excel
  const exportExcel = () => {
    const dataForExcel = dataTingkatan.map((t) => ({
      ID: t.id,
      'Kode Tingkatan': t.TINGKATAN_ID,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Data Tingkatan')
    XLSX.writeFile(wb, 'Laporan_Master_Tingkatan.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Master_Tingkatan')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Tingkatan"
      style={{ width: '50vw' }}
      modal
      blockScroll
    >
