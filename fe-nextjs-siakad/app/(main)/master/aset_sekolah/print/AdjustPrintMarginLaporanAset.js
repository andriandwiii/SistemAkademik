...
// 🔹 Komponen export laporan aset
export default function AdjustPrintMarginLaporanAset({
    adjustDialog,
    setAdjustDialog,
    dataAset = [],   // Data aset yang dikirim dari parent
    setPdfUrl,
    setFileName,
    setJsPdfPreviewOpen,
}) {
    ...
    async function exportPDF(adjustConfig) {
        ...
        const startY = addHeader(doc, 'LAPORAN MASTER ASET SEKOLAH', marginLeft, marginTop, marginRight)

        autoTable(doc, {
            startY: startY,
            head: [[
                'ID', 'Kode Aset', 'Nama Aset', 'Jenis Aset',
                'Jumlah', 'Kondisi', 'Gedung', 'Sumber Dana',
                'Tgl Pembelian', 'Harga Satuan', 'Total Harga',
                'Status', 'Keterangan'
            ]],
            body: dataAset.map(a => [
                a.ASET_ID,
                ...
                a.JUMLAH || 0,
                a.KONDISI || '-',
                a.gedung?.NAMA_GEDUNG || '-',
                a.SUMBER_DANA || '-',
                a.TANGGAL_PEMBELIAN
                    ? new Date(a.TANGGAL_PEMBELIAN).toLocaleDateString('id-ID')
                    : '-',
                a.HARGA_SATUAN?.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) || 'Rp0',
                a.TOTAL_HARGA?.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' }) || 'Rp0',
                a.STATUS || '-',
                a.KETERANGAN || '-'
            ]),
    ...
    // 🔸 Export Excel
    const exportExcel = () => {
        const dataForExcel = dataAset.map(a => ({
            'ID': a.ASET_ID,
            'Kode Aset': a.KODE_ASET,
            'Nama Aset': a.NAMA_ASET,
            'Jenis Aset': a.JENIS_ASET,
            'Jumlah': a.JUMLAH,
            'Kondisi': a.KONDISI,
            'Gedung': a.gedung?.NAMA_GEDUNG,
            'Sumber Dana': a.SUMBER_DANA,
            'Tanggal Pembelian': a.TANGGAL_PEMBELIAN
                ? new Date(a.TANGGAL_PEMBELIAN).toLocaleDateString('id-ID')
                : '-',
            'Harga Satuan': a.HARGA_SATUAN,
            'Total Harga': a.TOTAL_HARGA,
            'Status': a.STATUS,
        ...
        XLSX.writeFile(wb, 'Laporan_Master_Aset.xlsx')
    }
    ...
    const handleExportPdf = async () => {
        try {
            ...
            setFileName('Laporan_Master_Aset')
    ...
    return (
        <Dialog
            ...
            header="Pengaturan Cetak Laporan Aset"
            style={{ width: '50vw' }}
            modal
        >
