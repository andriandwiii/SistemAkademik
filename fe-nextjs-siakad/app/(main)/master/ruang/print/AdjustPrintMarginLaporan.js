...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    autoTable(doc, {
      startY,
      head: [['ID', 'Kode Ruang', 'Nama Ruang', 'Deskripsi']],
      body: dataRuang.map((r) => [
        r.id,
        r.RUANG_ID || '-',
        r.NAMA_RUANG || '-',
        r.DESKRIPSI || '-',
  ...
  // 🔸 Export Excel
  const exportExcel = () => {
    const dataForExcel = dataRuang.map((r) => ({
      ID: r.id,
      'Kode Ruang': r.RUANG_ID,
      'Nama Ruang': r.NAMA_RUANG,
    ...
    XLSX.writeFile(wb, 'Laporan_Master_Ruang.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Master_Ruang')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Ruang"
      style={{ width: '50vw' }}
      modal
      blockScroll
    >
