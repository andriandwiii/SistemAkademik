...
// 🔹 Komponen export laporan Kurikulum
export default function AdjustPrintMarginLaporanKurikulum({
  adjustDialog,
  setAdjustDialog,
  dataKurikulum = [], // data dari parent
  setPdfUrl,
  setFileName,
  setJsPdfPreviewOpen,
}) {
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN MASTER KURIKULUM', marginLeft, marginTop, marginRight)
  ...
  // 🔸 Export Excel
  const exportExcel = () => {
    const dataForExcel = dataKurikulum.map((k) => ({
      'ID': k.KURIKULUM_ID,
      'Nama Kurikulum': k.NAMA_KURIKULUM,
      'Tahun': k.TAHUN,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Data Kurikulum')
    XLSX.writeFile(wb, 'Laporan_Master_Kurikulum.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Master_Kurikulum')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Kurikulum"
      style={{ width: '50vw' }}
      modal
    >
