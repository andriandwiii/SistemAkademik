...
export default function AdjustPrintMarginTanggal({
  adjustDialog,
  setAdjustDialog,
  dataTanggal = [], // Data dari referensi_tanggal_rapor
  setPdfUrl,
  setFileName,
  setJsPdfPreviewOpen,
  dataAdjust,
  ...
  // 🔸 Export PDF khusus Referensi Tanggal Rapor
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(
      doc,
      'DAFTAR REFERENSI TANGGAL CETAK RAPOR',
      marginLeft,
      marginTop,
      marginRight
    )
    ...
    autoTable(doc, {
      startY,
      head: [['No', 'Semester', 'Tempat Cetak', 'Tanggal Cetak', 'Label KS', 'Label TTD']],
      body: dataTanggal.map((item, index) => [
        index + 1,
        item.SEMESTER || '-',
        item.TEMPAT_CETAK || '-',
        item.TANGGAL_CETAK || '-',
        item.TULISAN_KS || '-',
        `Kepsek: ${item.NIP_KEPSEK_LABEL} / Walas: ${item.NIP_WALAS_LABEL}`,
      ]),
  ...
  // 🔸 Export Excel khusus Referensi Tanggal Rapor
  const exportExcel = () => {
    const dataForExcel = dataTanggal.map((item, index) => ({
      No: index + 1,
      Semester: item.SEMESTER,
      'Semester Ke': item.SEMESTER_KE,
      'Tempat Cetak': item.TEMPAT_CETAK,
      'Tanggal Cetak': item.TANGGAL_CETAK,
      'Tulisan Jabatan KS': item.TULISAN_KS,
      'Label NIP Kepsek': item.NIP_KEPSEK_LABEL,
      'Label NIP Walas': item.NIP_WALAS_LABEL,
      'Status TTD Validasi': item.TTD_VALIDASI,
    }))

    const ws = XLSX.utils.json_to_sheet(dataForExcel)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Data Tanggal Rapor')
    XLSX.writeFile(wb, 'Laporan_Referensi_Tanggal_Rapor.xlsx')
  }
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Referensi_Tanggal_Rapor')
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Referensi Tanggal Rapor"
      style={{ width: '50vw' }}
      modal
      blockScroll
    >
